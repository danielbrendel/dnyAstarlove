...
window.vue = new Vue({
  ...
  methods: {
    ...
    showImagePreview: function showImagePreview(which, image, name) {
    ...
    displayNextPhoto: function displayNextPhoto(dir) {
    ...
    reportUser: function reportUser(id) {
      var reason = prompt(this.translationTable.enterReportReason, '');

      if (reason !== null) {
        this.ajaxRequest('post', window.location.origin + '/member/report/reason/' + id, {
          reason: reason
        }, function (response) {
          if (response.code == 200) {
            alert(response.msg);
