...
class ProfileDataModel extends Model
{
    ...
    {
        try {
            $item = static::where('userId', '=', $userId)->where('profile_item_ident', '=', $profile_item_ident)->first();
            if (!$item) {
                $item = new self();
                $item->userId = $userId;
    ...
    public static function querySingle($userId, $profile_item_name)
    {
        try {
            ...
            $dataItem = static::where('userId', '=', $userId)->where('profile_item_ident', '=', $profileItem->name)->first();
            if (!$dataItem) {
                return null;
            }
    ...
    public static function queryAll($userId)
    {
        try {
            ...
            foreach ($profileItem as $pi) {
                ...
                $dataItem = static::where('userId', '=', $userId)->where('profile_item_ident', '=', $pi->name)->first();
                if (!$dataItem) {
                    continue;
                }
